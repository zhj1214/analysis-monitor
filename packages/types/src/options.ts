import { Breadcrumb, BreadcrumbPushData } from './breadcrumb'
...
export interface Options {
  ...
  traceIdFieldName: string // 如果`enableTraceId`为true时，将会在所有请求头中添加`key`为`Trace-Id`，`value`为`uuid`的`traceId`，与`includeHttpUrlTraceIdRegExp`搭配使用
  ...
  maxDuplicateCount: number // 最多可重复上报同一个错误的次数
