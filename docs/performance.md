...
## 如何使用 @supaur/qdjk-web-performance 收集指标
...
// performance.js
// main.js 引入
import {WebVitals} from '@supaur/qdjk-web-performance'
...
面对这样的业务场景，@supaur/qdjk-web-performance也是具备自定义的能力。 在初始化的时候，增加customPaintMetrics属性，然后就可以调用wv.customCompletedPaint方法来自定义页面渲染的完成时间。
...
因为@supaur/qdjk-web-performance 中使用到了PerformanceObserver对象，所以在safari浏览器中不支持FP、FCP、LCP、FID指标的收集。
